...
export const fetchCurrentUser = () => (dispatch, getState) => {
  if (!HttpApi.hasToken) return null
...
export const requestInvitation = user =>
  createEffect(HttpApi.post('users/request_invitation', user))

// Update user

export const updateInfo = user =>
  createEffect(HttpApi.put('users/me', user), {
...
const userConnect = promise =>
  createEffect(promise, {
    before: setPosting(true),
    then: ({ token, user }) => () => {
