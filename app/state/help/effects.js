  ...
  dispatch(
    setContent(
      fetch(`/assets/help/${locale}/${pageName}.md`).then(response => {
        ...
        if (contentType && !contentType.includes('markdown')) throw ERROR_NOT_FOUND
        if (response.status === 200 || response.status === 304) return response.text()
        if (response.status === 404) return ERROR_NOT_FOUND
