...
const searchSpeakerRequest = debounce(
  (query) =>
    ...
    SocketApi.push("video_debate", "search_speaker", { query }).then(({ speakers }) => ({ options: speakers })),
  250,
)
...
export default class AddSpeakerForm extends React.PureComponent {
  render() {
    return (
      <form className="form">
        <Field name="speaker" component={(props) => this.renderSpeakerField(props)} disabled={this.props.disabled} />
      </form>
    )
  }
  ...
  onChange(e) {
    ...
    if (checkLength(e.full_name, SPEAKER_NAME_LENGTH)) addSpeaker(speaker).then(() => reset("add_speaker"))
  }
