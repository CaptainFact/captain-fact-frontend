...
class ActionDiff extends PureComponent {
  ...
  renderKeyDiff(key, changes) {
    ...
    if (changes.size === 2 && changes.first().removed && changes.last().added)
      return (
        <div>
          <span className="removed">{this.formatChangeValue(changes.first().value, key)}</span>,<span> -> </span>,
  ...
  generateDiff(allActions, action) {
    ...
    if (actionIdx + 1 < entityActions.size) prevState = this.buildReferenceEntity(entityActions.slice(actionIdx + 1))
  ...
  getActionChanges(action, prevState) {
    if ([ACTION_DELETE, ACTION_REMOVE].includes(action.type)) return prevState.map(() => null)
    if (action.type === ACTION_RESTORE) return prevState
  ...
  diffEntry(key, prevValue, newValue) {
    ...
    if (typeof newValue === "number") {
      ...
      // Generate diff
      if (prevValue) return [{ removed: true, value: prevValue }, { added: true, value: newValue }]
  ...
  formatValue(key, value) {
    ...
    if (key === "time") return value ? formatSeconds(value) : ""
    if (key === "source" || key === "url") return <ExternalLinkNewTab href={value}>{value}</ExternalLinkNewTab>
    return value
  }
}
