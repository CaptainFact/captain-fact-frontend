...
export class FlashMessages extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div id="flash-messages" onMouseEnter={() => this.props.pause()} onMouseLeave={() => this.props.unPause()}>
        <FlipMove enterAnimation="fade">
          {this.props.flashes.map((flash) => (
            <div key={flash.id} className={`flash-message is-${flash.flashType}`} data-timeleft={flash.timeLeft}>
              <button type="button" className="delete" onClick={() => this.props.removeFlash(flash)} />
