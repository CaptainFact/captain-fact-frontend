...
export default class StatementContainer extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div className={classNames("statement-container", { "is-focused": isFocused })} ref="container">
  ...
  shouldScroll = (props, prevProps) => {
    ...
    if (!this.isAutoScrollReady(props) || !this.isTarget(props)) return false
    ...
    if (props.autoscrollEnabled && !wasActive) return true

    // Only override autoscrollEnabled when we're forced by a scrollTo
    if (!props.autoscrollEnabled && (!wasActive || !wasForced) && this.isAutoScrollForced(props)) return true
