...
class CaptainFactSocketApi {
  ...
  joinChannel(identifier, channelAddress, mapEventsToFuncs = {}) {
    return new Promise((fulfill, reject) => {
      ...
      for (const [event, func] of Object.entries(mapEventsToFuncs)) channel.on(event, func)
      channel
        .join()
        .receive("ok", fulfill)
  ...
  leaveChannel(identifier) {
    ...
    if (!Object.keys(this.channels).length && ["connecting", "open"].includes(socketState)) this.socket.disconnect()
  }
